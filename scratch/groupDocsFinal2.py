...
pattern = r'@if \(\(\(List<GMK360\.Core\.Entities\.Construction\.ProjectLegalDocument>\)ViewBag\.Phase0Docs\) == null.*?\{\s*foreach \(var doc in \(\(List<GMK360\.Core\.Entities\.Construction\.ProjectLegalDocument>\)ViewBag\.Phase0Docs\)\.OrderByDescending\(x => x\.IsCustom\)\.ThenBy\(x => x\.Id\)\)\s*\{'

replace_block = '''@if (((List<GMK360.Core.Entities.Construction.ProjectLegalDocument>)ViewBag.Phase0Docs) == null || !((List<GMK360.Core.Entities.Construction.ProjectLegalDocument>)ViewBag.Phase0Docs).Any())
                              {
                                  <tr>
