...
$ctrlText = $ctrlText.Replace($oldGen, $newGen)

$regex = [regex]::new("public async Task<IActionResult> UpdateUnitProperties\(int Id, int BuildingId, string DoorNumber, string RoomLayout\).*?await _context\.SaveChangesAsync\(\);", [System.Text.RegularExpressions.RegexOptions]::Singleline)

$newMethod = @"
public async Task<IActionResult> UpdateUnitProperties(int Id, int BuildingId, string DoorNumber, string RoomLayout, string OwnerName, string OwnerPhone)
        {
...
$ctrlText = $regex.Replace($ctrlText, $newMethod)
