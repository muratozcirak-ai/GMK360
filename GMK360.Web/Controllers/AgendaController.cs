...
namespace GMK360.Web.Controllers
{
    ...
    public class AgendaController : Controller
    {
        ...
        {
            var agencyId = GetCurrentAgencyId();
            ViewBag.ProjectId = new SelectList(_context.ConstructionProjects.Where(p => p.AgencyId == agencyId), "Id", "Name");
            ViewBag.PhonebookId = new SelectList(_context.AgencyPhonebooks.Where(p => p.AgencyId == agencyId), "Id", "Name");
            return View(new AgendaRecord { EventDate = DateTime.Now });
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(AgendaRecord record, IFormFile imageFile)
        {
            var agencyId = GetCurrentAgencyId();
