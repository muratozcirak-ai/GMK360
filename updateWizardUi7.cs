...
class Program
{
    static void Main()
    {
        ...
        // Replace generateBlocks template
        int start = view.IndexOf("const html = `");
        int end = view.IndexOf("`;\r\n                  container.innerHTML += html;", start);
        if (end == -1) end = view.IndexOf("`;\n                  container.innerHTML += html;", start);

        if (start != -1 && end != -1)
        {
            string newTemplate = @"const html = `
            ...
            // Fix prepareSummary
            string oldSummary = @"blocks.forEach(b => {
                  totalApts += parseInt(b.querySelector('.b-apts').value) || 0;
                  totalShops += parseInt(b.querySelector('.b-shops').value) || 0;
              });";
            string newSummary = @"blocks.forEach(b => {
                  // We removed apts/shops from wizard, user adds them manually later in ManageBlock
              });";
              
            view = view.Replace(oldSummary, newSummary);

            File.WriteAllText(viewPath, view, new UTF8Encoding(true));
            Console.WriteLine("Wizard UI replaced successfully.");
        }
        else
        {
            Console.WriteLine("Failed to find start/end bounds.");
        }
    }
}
