...
class Program
{
    static void Main()
    {
        ...
        string find1 = @"public async Task<IActionResult> SaveStep1\(\[FromForm\] GMK360\.Web\.Models\.CreateProjectWizardViewModel model\)\s*\{.*?return Json\(new \{ success = true, draftId = project\.Id \}\);\s*\}";
        string replace1 = @"public async Task<IActionResult> SaveStep1([FromForm] GMK360.Web.Models.CreateProjectWizardViewModel model)
        {
            try {
        ...
        string find2 = @"public async Task<IActionResult> SaveStep2\(\[FromForm\] GMK360\.Web\.Models\.CreateProjectWizardViewModel model\)\s*\{.*?return Json\(new \{ success = true \}\);\s*\}";
        string replace2 = @"public async Task<IActionResult> SaveStep2([FromForm] GMK360.Web.Models.CreateProjectWizardViewModel model)
        {
            try {
        ...
        code = Regex.Replace(code, find1, replace1, RegexOptions.Singleline);
        code = Regex.Replace(code, find2, replace2, RegexOptions.Singleline);
        
        File.WriteAllText(path, code, new UTF8Encoding(true));
        Console.WriteLine("Added try-catch to SaveStep methods.");
    }
}
